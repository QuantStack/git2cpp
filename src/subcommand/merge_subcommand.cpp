...
merge_subcommand::merge_subcommand(const libgit2_object&, CLI::App& app)
{
    ...
    // sub->add_flag("--no-ff", m_no_ff, "Create a merge commit in all cases, even when the merge could
    // instead be resolved as a fast-forward."); sub->add_flag("--commit", m_commit, "Perform the merge and
    // commit the result. This option can be used to override --no-commit.");
    sub->add_flag(
        "--no-commit",
        m_no_commit,
...
)
{
    ...
    auto author_committer_sign_now = signature_wrapper::signature_now(
        author_name,
        author_email,
        author_name,
        author_email
...
void merge_subcommand::run()
{
    ...
    if (state == GIT_REPOSITORY_STATE_MERGE)
    {
        ...
        else if (m_quit)
        {
            ...
            // if (m_continue)
            // {
            //     std::cout << "fatal: --abort expects no arguments" << std::endl;   // TODO: add the help
            //     info return;
            // }
        ...
        else if (m_continue)
        {
            ...
            {
                // std::string commit_message = "Merge branch ";    // how to get the name of the branch the
                // merge was started on ? auto author_committer_signatures =
                // signature_wrapper::get_default_signature_from_env(repo);
                ...
                std::cout << "Merge made" << std::endl;  // TODO: change the outpout to something like this:
                                                         // 3c22161 (HEAD -> master) Merge branch 'foregone'
        ...
        std::cout << yellow << "hint: Fix them up in the work tree, and then use 'git add/rm <file>'"
                  << std::endl;
        std::cout << "hint: as appropriate to mark resolution and make a commit." << termcolor::reset
                  << std::endl;
