...
def test_merge_fast_forward(repo_init_with_commit, commit_env_config, git2cpp_path, tmp_path):
    ...
    p_merge = subprocess.run(merge_cmd, capture_output=True, cwd=tmp_path, text=True)
    assert p_merge.returncode == 0
    assert "Fast-forward" in p_merge.stdout

    log_cmd = [git2cpp_path, "log", "--format=full", "--max-count", "1"]
    p_log = subprocess.run(log_cmd, capture_output=True, cwd=tmp_path, text=True)
...
def test_merge_commit(repo_init_with_commit, commit_env_config, git2cpp_path, tmp_path):
    ...
    p_add_2 = subprocess.run(add_cmd_2, capture_output=True, cwd=tmp_path, text=True)
...
def test_merge_conflict(
    ...
    if flag == "--abort":
        ...
        p_branch_alias = subprocess.run(
            branch_alias_cmd, capture_output=True, cwd=tmp_path, text=True
        )
