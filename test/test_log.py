...
def test_max_count(
    repo_init_with_commit, commit_env_config, git2cpp_path, tmp_path, max_count_flag
):
    assert (tmp_path / "initial.txt").exists()

    p2 = tmp_path / "second.txt"
    p2.write_text("second file")

    cmd_add2 = [git2cpp_path, "add", "second.txt"]
    ...
    cmd_add3 = [git2cpp_path, "add", "third.txt"]
...
def test_log_with_head_reference(
    ...
    p_log = subprocess.run(
        [git2cpp_path, "log", "-n", "1"], capture_output=True, cwd=tmp_path, text=True
    )
    assert p_log.returncode == 0
...
def test_log_with_tag(commit_env_config, git2cpp_path, tmp_path):
...
def test_log_with_multiple_tags(commit_env_config, git2cpp_path, tmp_path):
    ...
    subprocess.run([git2cpp_path, "tag", "v1.0.0"], cwd=tmp_path, check=True)
    subprocess.run([git2cpp_path, "tag", "stable"], cwd=tmp_path, check=True)
    subprocess.run([git2cpp_path, "tag", "latest"], cwd=tmp_path, check=True)
...
def test_log_with_annotated_tag(commit_env_config, git2cpp_path, tmp_path):
...
def test_log_with_branch(commit_env_config, git2cpp_path, tmp_path):
...
def test_log_commit_without_references(commit_env_config, git2cpp_path, tmp_path):
    ...
    p_log = subprocess.run(
        [git2cpp_path, "log", "-n", "2"], capture_output=True, cwd=tmp_path, text=True
    )
    assert p_log.returncode == 0
